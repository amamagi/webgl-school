...
export class WebGLUtility {
  ...
  static loadImage(path) {
    return new Promise((resolve) => {
      // Image オブジェクトの生成
      const img = new Image();
      img.crossOrigin = 'anonymous'; // CORS を許可する
   ...
   * floatフレームバッファを生成する
   */
  static createFloatFramebuffer(gl, width, height, channels=4){
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, texture);
    const format = (channels === 4) ? gl.RGBA 
                 : (channels === 3) ? gl.RGB 
                 : (channels === 2) ? gl.RG
                 : gl.RED;
    const internalFormat = (channels === 4) ? gl.RGBA32F
                         : (channels === 3) ? gl.RGB32F
                         : (channels === 2) ? gl.RG32F
                          : gl.R32F;
    ...
    //   console.error(`Float framebuffer creation failed with status: ${status}\nChannels: ${channels}, Format: ${format}, Internal Format: ${internalFormat}\nWidth: ${width}, Height: ${height}`);
    ...
    //   gl.bindTexture(gl.TEXTURE_2D, null);
    //   gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    //   throw new Error(`Float framebuffer creation failed: ${status}`);
    // }

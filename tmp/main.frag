...
void main() {
    // FLOATテクスチャから値を読み出し
    vec4 floatValue = texture(u_floatTexture, vTexCoord);
    
    // 正規化（minValue ~ maxValue を 0.0 ~ 1.0 にマッピング）
    float normalized = (floatValue.r - u_minValue) / (u_maxValue - u_minValue);
    ...
    if (u_visualizationMode == 0) {
        ...
        // ヒートマップ
        color = heatmap(normalized);
    } else {
        // 生のRGB値（floatValue.rgb をそのまま表示）
        color = floatValue.rgb;
    }
